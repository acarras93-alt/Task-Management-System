...
from task_v4 import InvalidTaskStatusError, Task
...
def test_empty_title_raises_value_error():
    with pytest.raises(ValueError):
        Task(
            task_id=1, title="", description="Practice pytest basics", status="pending"
        )


def test_empty_description_raises_value_error():
    with pytest.raises(ValueError):
        Task(task_id=1, title="Study Python", description="", status="pending")
...
def test_update_description_changes_task_description():
    task = Task(
        task_id=1, title="Study Python", description="Old description", status="pending"
    )
