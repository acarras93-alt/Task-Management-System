...
class InMemoryTaskRepository(TaskRepository):
    ...
    def update(self, task: Task) -> None:
        if task.task_id not in self._tasks_by_id:
            raise TaskNotFoundError(f"Task with ID {task.task_id} was not found.")
...
class JSONTaskRepository(TaskRepository):
    ...
    def list_all(self) -> list[Task]:
        ...
        try:
            ...
            raise TaskStorageError(f"Invalid product data type: {error}.") from error

        except ValueError as error:
            raise TaskStorageError(f"Invalid product value: {error}.") from error
...
class TaskService:
    ...
    def add_task(self, task_id: int, title: str, description: str, status: str) -> Task:
        task = Task(
    ...
    def update_details(
        self, task_id: int, new_title: str, new_description: str
    ) -> Task:
        task = self._repository.get_by_id(task_id)
